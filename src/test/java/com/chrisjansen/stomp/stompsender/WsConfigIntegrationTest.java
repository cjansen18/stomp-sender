...
import com.chrisjansen.stomp.stompsender.config.WsConfig;
import com.chrisjansen.stomp.stompsender.controllers.RegisterController;
    ...
    public void receivesMessageFromSubscribedQueue() throws Exception {
        ...
        stompSession1.subscribe(WsConfig.SUBSCRIBE_QUEUE,
            new ClientFrameHandler((payload) -> {
                log.info("--> "+WsConfig.SUBSCRIBE_QUEUE+" (client 1 /queue) : "+payload);
                queue1.offer(payload.toString());
            }));
        stompSession1.subscribe(WsConfig.SUBSCRIBE_USER_PREFIX+WsConfig.SUBSCRIBE_USER_REPLY,
            new ClientFrameHandler((payload) -> {
                log.info("--> "+WsConfig.SUBSCRIBE_USER_PREFIX+WsConfig.SUBSCRIBE_USER_REPLY+" (client 1 /private/reply) : "+payload);
        ...
        stompSession2.subscribe(WsConfig.SUBSCRIBE_QUEUE,
            new ClientFrameHandler((payload) -> {
                log.info("--> "+WsConfig.SUBSCRIBE_QUEUE+" (client2 /queue) : "+payload);
                queue2.offer(payload.toString());
            }));
        stompSession2.subscribe(WsConfig.SUBSCRIBE_USER_PREFIX+WsConfig.SUBSCRIBE_USER_REPLY,
            new ClientFrameHandler((payload) -> {
                log.info("--> "+WsConfig.SUBSCRIBE_USER_PREFIX+WsConfig.SUBSCRIBE_USER_REPLY+" (client 2 /private/reply) : "+payload);
        ...
        stompSession1.send(RegisterController.ENDPOINT_REGISTER, "hello everyone!");
        Thread.currentThread().sleep(100);
        Assert.assertEquals("Thanks for your registration!", userQueue1.poll());
        Assert.assertEquals("Someone just registered saying: hello everyone!", queue1.poll());
        Assert.assertEquals("Someone just registered saying: hello everyone!", queue2.poll());

        Thread.currentThread().sleep(100);

        log.info("### client2 registers");
        stompSession2.send(RegisterController.ENDPOINT_REGISTER, "what's the good word, people!");
        Thread.currentThread().sleep(100);
        Assert.assertEquals("Thanks for your registration!", userQueue2.poll());
        Assert.assertEquals("Someone just registered saying: what's the good word, people!", queue1.poll());
        Assert.assertEquals("Someone just registered saying: what's the good word, people!", queue2.poll());
    }
